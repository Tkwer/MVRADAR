...
class MultiViewFeatureFusion(nn.Module):  
    ...
    def __init__(self, backbone="custom", cnn_output_size=128, hidden_size=128,   
                 rnn_type='lstm', lstm_layers=1, bidirectional=True, fc_size=128,   
                 input_feature_shapes=None, fusion_mode='concatenate', alignment_type='attention',  
                 sharedspecific_method='basic_shared', bottleneck_dim=None, selected_features=None):  
        super(MultiViewFeatureFusion, self).__init__()  

        self.fusion_mode = fusion_mode  # Fusion mode (concatenate/alignment/shared_specific)  
        self.alignment_type = alignment_type  # For AlignmentDecoder (e.g., 'attention', 'mi', etc.)  
        self.sharedspecific_method = sharedspecific_method  # For SharedSpecificDecoder (e.g., 'basic_shared', etc.)  
        ...
        if fusion_mode == 'concatenate':  
            ...
            self.decoder = AlignmentDecoder(feature_dim, num_views=len(self.selected_features), alignment_type=alignment_type)  
            ...
            self.decoder = SharedSpecificDecoder(  
                feature_dim, num_views=len(self.selected_features), method=sharedspecific_method, bottleneck_dim=bottleneck_dim  
            )  
        else:  
